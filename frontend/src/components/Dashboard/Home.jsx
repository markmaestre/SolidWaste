...
const RATE_LIMIT = {
  ...
  COOLDOWN_MS: 60000, // 1 minute between submissions
};
...
const HomeScreen = ({ navigation }) => {
  ...
  const checkRateLimit = () => {
    ...
    // Check cooldown (1 minute between submissions)
    if (now - lastSubmissionTime < RATE_LIMIT.COOLDOWN_MS) {
      const remainingSeconds = Math.ceil((RATE_LIMIT.COOLDOWN_MS - (now - lastSubmissionTime)) / 1000);
      Alert.alert('Rate Limit', `Please wait ${remainingSeconds} seconds before sending another message.`);
      return false;
    }
    
    // Check hourly limit
    if (submissionCount >= RATE_LIMIT.MAX_SUBMISSIONS_PER_HOUR) {
      Alert.alert('Rate Limit', 'Too many messages sent. Please try again later.');
      return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    if (!EMAILJS_SERVICE_ID || !EMAILJS_TEMPLATE_ID || !EMAILJS_PUBLIC_KEY) {
      Alert.alert('Error', 'Email service is not properly configured. Please contact support.');
      console.error('EmailJS configuration missing from .env');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        Alert.alert('Success', 'Message sent! We will get back to you within 24 hours.');
        setForm({ name: '', email: '', phone: '', organization: '', message: '' });
      } else {
      ...
      Alert.alert(
        'Error',
        'Failed to send message. Please try again or contact us directly at solidwaste@gmail.com'
      );
    } finally {
      setSending(false);
    }
  ...
  return (
    <SafeAreaView style={s.root}>
      ...
      <ScrollView ref={scrollRef} style={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        {activeTab === 'home' && (
          <>
            {/* Hero — full dark block that includes the scroll hint */}
            <View style={s.heroBg}>
              <View style={s.blob1} />
              <View style={s.blob2} />
              <View style={s.blob3} />
              ...
              {/* Scroll hint — INSIDE the dark heroBg so no white gap */}
              <View style={s.scrollHint}>
                <View style={s.scrollHintLine} />
                <Text style={s.scrollHintTxt}>Why SolidWaste</Text>
                <View style={s.scrollHintLine} />
            ...
            {/* Features — light section starts cleanly after the full dark block */}
            <View style={s.section}>
              <FadeIn>
                <Badge label="Core Features" light />
                <Text style={s.sectionTitle}>Built for Scale.{'\n'}Designed for Impact.</Text>
        ...
        {activeTab === 'services' && (
          <>
            ...
            <View style={{ backgroundColor: C.offWhite, paddingVertical: 8 }}>
              <FlatList
                data={SERVICES}
                keyExtractor={(item) => item.n}
                horizontal
        ...
        {activeTab === 'contact' && (
          <View style={s.section}>
            ...
            <FadeIn delay={120}>
              <InfoCard title="Send Us a Message" iconName="send-outline">
                <Field label="Full Name"     value={form.name}         onChange={setField('name')}         placeholder="Your full name"              caps="words"             required />
                <Field label="Email Address" value={form.email}        onChange={setField('email')}        placeholder="your@email.com"              keyboard="email-address" caps="none" required />
                <Field label="Phone Number"  value={form.phone}        onChange={setField('phone')}        placeholder="+63 XXX XXX XXXX"            keyboard="phone-pad"     caps="none" />
                <Field label="Organisation"  value={form.organization} onChange={setField('organization')} placeholder="Company or organisation"     caps="words" />
                ...
                <TouchableOpacity
                  ...
                  disabled={sending}
                >
                  <Text style={s.submitTxt}>{sending ? 'Sending…' : 'Send Message'}</Text>
                  {!sending && (
                    <Ionicons name="send-outline" size={15} color={C.navy} style={{ marginLeft: 8 }} />
                  )}
                </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  scroll: { flex: 1, backgroundColor: C.ink },
  ...
  scrollHintLine: { flex: 1, height: 1, backgroundColor: 'rgba(255,255,255,0.12)' },
  scrollHintTxt:  { fontSize: 11, color: C.slateL, fontWeight: '600', letterSpacing: 0.8, textTransform: 'uppercase' },

  // ── Section ─────────────────────────────────────────────────────────────────
  section:      { paddingHorizontal: 20, paddingVertical: 24, backgroundColor: C.offWhite },
  sectionTitle: {
    fontSize: 30, fontWeight: '900', color: C.navy,
    letterSpacing: -0.6, lineHeight: 38, marginBottom: 24,
  },
  ...
  // ── Services – horizontal scroll cards ──────────────────────────────────────
  servicesScroll: { paddingHorizontal: 20, paddingBottom: 8, gap: 14, backgroundColor: C.offWhite },
  serviceCard: {
    width: width * 0.68, backgroundColor: C.navy,
    borderRadius: 18, padding: 22, borderWidth: 1, borderColor: C.navyMid,
  },
